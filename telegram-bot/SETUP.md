...
5. **Configure TradingView webhook:**
   - Right-click chart → Add alert
   - Condition: your indicator → "Any alert() function call"
   - Notifications tab: **Webhook URL** = `https://tv-bot.onrender.com/webhook`
   - Message field (Alert message):
     ```json
     {"secret":"mySecretPass123","text":"{{message}}"}
     ```
   - Save
...
| Webhook returns 401 | Secret in TradingView message doesn't match env var |
| Render sleeps after 15 min | Free tier limitation — use UptimeRobot to ping every 5 min |
